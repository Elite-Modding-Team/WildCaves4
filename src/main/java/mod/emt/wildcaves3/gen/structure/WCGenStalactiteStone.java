package mod.emt.wildcaves3.gen.structure;
...
public class WCGenStalactiteStone {
    public final Block blockId;

    public WCGenStalactiteStone() {
        this(WCBlocks.stalactite_stone);
    }

    public WCGenStalactiteStone(Block toGen) {
        blockId = toGen;
    }
    ...
    public void generate(World world, Random random, BlockPos pos, int distance, int maxLength) {
        ...
        if (distance <= 1) {
            ...
            // stalagmite base
            if (!world.getBlockState(botY).getMaterial().isLiquid() && WCWorldGen.isWhiteListed(world.getBlockState(botY.down()).getBlock())) {
                aux = WCUtils.randomChoice(-1, 8, 9, 10);
                if (aux != -1) {
            ...
            if (j == 2) {
                ...
                while (k < maxLength && topY.getY() >= botY.getY() && j < distance && !world.getBlockState(topY.down()).getMaterial().isLiquid()) {
                    ...
                    if (world.isAirBlock(topY) && topMetadata > 2 && topMetadata < 6) {
                        ...
                        if (aux != 4)
                            world.setBlockState(topY, blockId.getStateFromMeta(WCUtils.randomChoice(4, 5, 7, 11)), 2);
                        else
                            world.setBlockState(topY, blockId.getStateFromMeta(WCUtils.randomChoice(7, 11)), 2);
                        j++;
                    }
                    // Expand upwards
                    if (world.isAirBlock(botY) && (bottomMetadata > 3 && bottomMetadata < 5 || bottomMetadata == 8) && j < distance && stalagmiteGenerated) {
                        aux = random.nextInt(5);
                        if (aux != 4)
                            world.setBlockState(botY, blockId.getStateFromMeta(WCUtils.randomChoice(4, 5, 6, 12)), 2);
                        else
                            world.setBlockState(botY, blockId.getStateFromMeta(WCUtils.randomChoice(12, 6)), 2);
                        j++;
                    }
                }
            }
    ...
    protected void generateStalactiteBase(World world, Random random, BlockPos topY) {
        world.setBlockState(topY, blockId.getStateFromMeta(WCUtils.randomChoice(1, 2, 3, 3)), 2);
    }
}
